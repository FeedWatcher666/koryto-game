...
const fixture = {
  day: 4,
  quests: {
    register:{status:'active',deadlineBonus:0},
    roof:{status:'active',deadlineBonus:0},
    meadow:{status:'active',deadlineBonus:0}
  },
  factionPlans: {
    oldguard:{progress:61},
    business:{progress:38}
  },
  party: {
    marie:{name:'Marie',icon:'👩‍🏫',loyalty:35},
    bohumil:{name:'Bohumil',icon:'🗄️',loyalty:64}
  },
  companionAmbitions:{marie:{tension:48},bohumil:{tension:4}},
  flags:{},ui:{},stats:{heat:20,integrity:60},debt:0,promiseSummary:{}
};
const snapshot = api.prioritySnapshot(fixture, {
  questDefs: context.questDefs,
  questDeadline: id => context.questDefs[id].deadline,
  factionPlanDefs: context.KorytoFactionData.factionPlanDefs,
...
const election = api.electionAnalysis({
  electionBreakdown:[
    {name:'Rodiče',icon:'🎒',votes:100,ballots:150,share:66.7},
    {name:'Senioři',icon:'🧓',votes:55,ballots:160,share:34.4}
  ],
  promiseSummary:{broken:2},debt:4,stats:{heat:52,integrity:31}
});
...
assert.match(index, /0\.14\.5 TEST\.10/);
...
assert.equal(readText('VERSION').trim(),'0.14.5-test.10');
console.log('v0.14.5 playability and 1000-campaign audit ok');
