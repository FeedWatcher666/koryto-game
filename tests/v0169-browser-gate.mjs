...
const dist = path.resolve(process.argv[2] || 'dist/koryto-v0.16.9-test.3');
...
try {
  for (const target of targets) {
    ...
    const auditSurface = async label => {
      const audit = await page.evaluate(() => {
        ...
        return {
          ...
          footerVersionCount: (footer.match(/0\.16\.9 TEST\.3/g) || []).length,
      ...
      assert.match(audit.title, /0\.16\.9 TEST\.3/, `${target.name} ${label}: title`);
      assert.match(audit.meta, /0\.16\.9 TEST\.3/, `${target.name} ${label}: meta`);
      assert.equal(audit.build, '0.16.9-test.3', `${target.name} ${label}: data build`);
      return audit;
    };
    ...
    try {
      ...
      await page.waitForFunction(() => globalThis.KorytoBuildInfo?.buildVersion === '0.16.9-test.3');
      ...
      assert.match(identity.title, /0\.16\.9 TEST\.3/, `${target.name}: title`);
      assert.match(identity.meta, /0\.16\.9 TEST\.3/, `${target.name}: meta`);
      assert.equal(identity.build, '0.16.9-test.3', `${target.name}: data build`);
      assert.match(identity.brand, /0\.16\.9 TEST\.3/, `${target.name}: brand`);
