...
assert.equal(context.KorytoReleaseCandidate.VERSION,'0.14.4 TEST.10');
assert.equal(context.KorytoReleaseCandidate.BUILD_VERSION,'0.14.4-test.10');
assert.equal(context.KorytoReleaseCandidate.SAVE_VERSION,'0.14.3-test.2');
const app=readText('src/app.js');
assert.ok(app.split('\n').length<1000,'app.js must stay below 1000 lines after physical extraction');
...
for(const name of ['KorytoCoreData','KorytoEventData','KorytoQuestData','KorytoCompanionData','KorytoFactionData','KorytoDebateData','KorytoQuestRuntime','KorytoEventSystem','KorytoFactionSystem','KorytoCompanionSystem','KorytoDebateSystem','KorytoElectionSystem','KorytoGameEngine','KorytoBalanceSystem','KorytoApp'])assert.ok(context[name],`${name} missing`);
...
assert.equal(loaded.hero.name,'Legacy 0.13');
assert.equal(loaded.version,'0.14.3-test.2');
assert.equal(loaded.saveSchema,1);
assert.equal(loaded.quests.register.stage,0);
assert.equal(loaded.quests.register.deadlineBonus,1);
...
console.log('v0.14.4 modular architecture ok');
