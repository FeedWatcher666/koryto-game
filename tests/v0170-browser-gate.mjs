...
const dist = path.resolve(process.argv[2] || 'dist/koryto-v0.17.0-test.1');
...
try {
  for (const target of targets) {
    ...
    const auditSurface = async label => {
      const audit = await page.evaluate(() => {
        ...
        return {
          ...
          footerVersionCount: (footer.match(/0\.17\.0 TEST\.1/g) || []).length,
      ...
      assert.match(audit.title, /0\.17\.0 TEST\.1/, `${target.name} ${label}: title`);
      assert.match(audit.meta, /0\.17\.0 TEST\.1/, `${target.name} ${label}: meta`);
      assert.equal(audit.build, '0.17.0-test.1', `${target.name} ${label}: data build`);
      assert.equal(audit.visual, '0.17.0-test.1', `${target.name} ${label}: visual marker`);
    ...
    try {
      ...
      await page.waitForFunction(() => globalThis.KorytoBuildInfo?.buildVersion === '0.17.0-test.1' && globalThis.KorytoUI170?.visualAudit?.().active);
      ...
      assert.match(identity.title, /0\.17\.0 TEST\.1/, `${target.name}: title`);
      assert.match(identity.meta, /0\.17\.0 TEST\.1/, `${target.name}: meta`);
      assert.equal(identity.build, '0.17.0-test.1', `${target.name}: data build`);
      assert.equal(identity.visual, '0.17.0-test.1', `${target.name}: visual data build`);
      assert.match(identity.brand, /0\.17\.0 TEST\.1/, `${target.name}: brand`);
...
console.log('v0.17.0 TEST.1 packaged Chromium visual foundation and complete save/load gate passed');
