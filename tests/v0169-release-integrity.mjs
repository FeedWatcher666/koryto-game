...
const browserGate = read('tests/v0169-browser-gate.mjs');

assert.equal(read('VERSION').trim(), '0.16.9-test.3');
for (const token of ['0.16.9 TEST.3','0.16.9-test.3','0.14.3-test.2']) assert.match(buildInfo, new RegExp(token.replaceAll('.', '\\.')));
assert.match(buildInfo, /globalThis\.KorytoBuildInfo/);
assert.match(buildInfo, /applyLabels/);
assert.match(buildInfo, /\(\?:\\s\*·\\s\*0/);
assert.match(html, /<title>Koryto 0\.16\.9 TEST\.3/);
assert.match(html, /src\/build-info\.js/);
assert.match(html, /Dolní Vejprnice 0\.16\.9 TEST\.3/);
const staticFooter = html.match(/<div class="footer-note">([^<]*)<\/div>/)?.[1] || '';
assert.doesNotMatch(staticFooter, /0\.16\.9 TEST\.3/);
...
assert.match(workflow, /0\.16\.9-test\.3/);
assert.match(browserGate, /save\/load roundtrip state/);
...
assert.doesNotMatch(html + buildInfo + guard + release + mapRuntime + campaignRuntime + campaignCss + navCss, /https?:\/\/(?:fonts|cdn|unpkg|jsdelivr)/);

console.log('v0.16.9 TEST.3 release integrity closure contract passed');
