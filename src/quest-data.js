...
const definitions={
 ...
 ballots:{title:"Hlasovací lístky bez kandidáta",deadline:13,desc:"Tiskárna vynechala vaše jméno a dvakrát vytiskla Věčného.",location:"townhall",failure:"Chyba se opraví pozdě a část voličů dostane návod k volbě místo lístku."}
  ...
  function validate(locations = globalThis.KorytoCoreData?.locations || {}) {
    const issues=[];
    for(const [id,def] of Object.entries(definitions)){
      ...
      if(!locations[def.location])issues.push(`${id}: neznámá lokace ${def.location}`);
    }
    return issues;
  }
  globalThis.KorytoQuestData={VERSION,BUILD_VERSION,definitions,cloneDefinitions:()=>clone(definitions),validate};
})();
