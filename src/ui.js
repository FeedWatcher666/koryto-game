...
export function gameView(state) {
  return `<div class="game-screen">${hud(state)}<main class="game-layout">${heroPanel(state)}<section class="world-stage">${sceneView(state)}</section>${partyPanel(state)}</main><footer>Čistý runtime v0.20 · první vícefázový quest · jediný renderer · save schema 2</footer></div>`;
}
